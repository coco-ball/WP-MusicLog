        ...
        <div key={index} className="pb-8">
          <div className="flex bg-white border-2 border-black w-auto h-auto hover:scale-105 hover:border-[#617FF5] transition-transform">
            ...
            <div key={log.id} className="w-3/4 bg-white p-4 relative">
              <p className="text-2xl font-bold text-xl mb-6">{log.title} - {log.artist}</p>
              <p className="text-l font-bold" style={{ display: "inline-block" }}>위치: </p> {log.location}
              <p className="mb-4"></p>
              <p className="text-l font-bold" style={{ display: "inline-block" }}>시간: </p> {log.datetime}
              <p className="mb-4"></p>
              ...
              <button>
                <img
                  ...
                  className={
                    "absolute top-0 right-0 w-6 mt-2 mr-2 text-black font-serif hover:bg-white hover:bg-[#617FF5] text-xs"
                  }
                ></img>
              </button>
            </div>
