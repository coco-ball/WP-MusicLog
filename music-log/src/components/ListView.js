    ...
    <div className = "w-full">
      {logs.map((log, index) => (
        <div key={index} className="pt-4 pb-4">
          <div className="flex bg-white border-2 border-black w-auto">
            ...
            <div key={log.id} className="w-4/5 bg-white p-4 relative">
              <button>
                <img
                  src="/close.svg"
                  onClick={() => deleteLog(log.id)}
                  className={
                    "absolute top-0 right-0 w-4 mt-2 text-black font-serif hover:bg-white hover:text-cyan-700 text-xs"
                  }
                ></img>
              </button>
              <p className="text-xl font-bold text-xl mb-1">{log.title}</p>
              <p className="text-l text-small">{log.artist}</p>
